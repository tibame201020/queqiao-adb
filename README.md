...
The extension does not accept arbitrary shell text. ADB commands are assembled as validated argv and executed by the Worker-owned extension runtime with `shell: false`. Its runtime manifest only allows `adb`, `adb.exe`, and known MuMu ADB executable paths. Package names, device IDs, coordinates, durations, and key codes are schema-bounded.
...
Requires Node.js 22.19–24 and Queqiao 0.9.7 extension API.
